...
try:
    db_dir = os.path.join (os.getenv('IBUS_INDIC_TABLE_LOCATION'),'indic-tables')
    icon_dir = os.path.join (os.getenv('IBUS_INDIC_TABLE_LOCATION'),'icons')
except:
    db_dir = "/usr/share/ibus-indic-table/indic-tables"
    icon_dir = "/usr/share/ibus-indic-table/icons"
...
opt.add_option('--no-debug', '-n',
        ...
        help = 'redirect stdout and stderr to ~/.ibus/indic-tables/debug.log, default: %default')
...
if (not options.xml) and options.debug:
    if not os.access ( os.path.expanduser('~/.ibus/indic-tables'), os.F_OK):
        os.system ('mkdir -p ~/.ibus/indic-tables')
    logfile = os.path.expanduser('~/.ibus/indic-tables/debug.log')
...
class IMApp:
    def __init__(self, dbfile, exec_by_ibus):
        ...
        if exec_by_ibus:
            self.__bus.request_name("org.freedesktop.IBus.IndicTable", 0)
        else:
            self.__component = ibus.Component("org.freedesktop.IBus.IndicTable",
                                              "Table Component",
                                              "0.1.0",
                                              "GPL",
                                              "Naveen Kumar <nav007@gmail.com>")
